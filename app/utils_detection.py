...
def detect_segment_point_input_prompts(bbox, zoom, point_coords, point_labels, id, project):
    ...
    try:
        ...
        logging.info(f"Segmenting image based on point inputs at {output_image_path} for project {project} with ID {id}")
        sam.set_image(output_image_path)
        sam.predict(point_coords, point_labels=point_labels, point_crs="EPSG:4326", output=mask_file_path)
